...
function App() {
  ...
  useEffect(() => {
    ...
    return () => {
      document.body.removeEventListener('gz-search-open', customOpenEventListener);
  ...
  if (show) {
    return <>
      ...
      <div role="dialog" onClick={() => setShow(false)} style={{position: 'fixed',
        top: 0,
        left: 0,
        zIndex: 1050,
        display: 'block',
        ...
        <div className="modal" style={{
          ...
          position: 'relative',
          width: 'auto',
          maxWidth: 'min(53.75rem, calc(100vw - var(--lia-bs-grid-gutter-width)))'
        }}>
          <form onClick={(event) => {
            event.stopPropagation()
          }}>
            <input placeholder="Custom search" style={{
              width: '100%',
              padding: '20px',
              border: 0,
